...
download_model() {
  ...
  if [[ "$model_url" == huggingface://* ]]; then
    ...
    warn "Cannot download non-HuggingFace URL $model_url without Docker support. Skipping."
    return 1
  else
    # For non-HuggingFace URLs on supported platforms, use Docker
    info "Downloading $model_name using Docker"
    docker run --rm \
...
if [ "$CAN_USE_DOCKER" = true ]; then
  ...
  echo "Docker support: disabled (using direct downloads only)"
fi
echo
